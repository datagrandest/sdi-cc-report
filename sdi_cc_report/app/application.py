...
class Application:
    ...
    def __init__(self, config_file=None, app=None):
        ...
        self.add_logs(
            message='Config file "{config_file}" loaded.'.format(
                config_file=self.config_file
            )
        )
        self.add_logs(
            message='Locales file "{locales_file}" loaded (language "{lang}")'.format(
                locales_file=self.locales_file, lang=self.translate["lang"]
            )
        )
    ...
    def load_config(self, config_file=None):
        if config_file is not None and os.path.isfile(config_file):
            ...
            self.locales_file = os.path.abspath(
                os.path.join(
                    self.root_dir,
                    self.config["locales"]["directory"],
                    self.config["locales"]["lang"] + ".yaml",
                )
            )
            self.log_file = os.path.abspath(
                os.path.join(self.root_dir, self.config["log"]["log_file"])
            )
            self.dashboard_templates_directory = os.path.abspath(
                os.path.join(
                    self.root_dir, self.config["dashboard"]["templates_directory"]
                )
            )
            ...
            print(
                'ERROR: config file "{file}" cannot be loading.'.format(
                    file=config_file
                )
            )
            self.on_exit_app()
    ...
    def set_logs(self, message="", level="INFO"):
        if self.log_file is None:
            log_message = (
                "{now} - {level}: Log file is not define in config.yaml".format(
                    now=now, level="ERROR"
                )
            )
        ...
        log_message = "{now} - {level}: {message}".format(
            now=now, level=level, message=message
        )
        self.logs.append(log_message)
    ...
    def get_logs(self, search=None, limit=10):
        where = (
            [s.strip() for s in "+".join(search).split("+")]
    ...
    def get_errors(self, report=None, filter=None, workspace=None, name=None, id=None):
        ...
        r = Report(
            ...
            title=report["name"],
            type=report["type"],
            ssl_verify=self.config["requests"]["ssl_verify"],
        )
        data = r.get_errors(filter=filter, workspace=workspace, name=name, id=id)

        return {"nb_errors": r.nb_errors, "errors": data.errors}
    ...
    def get_layers(self, report=None, filter=None, workspace=None, name=None, id=None):
        ...
        return {"nb_layers": r.nb_layers, "layers": data.layers}
    ...
    def get_workspaces(self, report=None, filter=None):
        ...
        return {"nb_workspaces": r.nb_workspaces, "workspaces": data.workspaces}
    ...
    def generate_dashboard(
        self, report, destination=None, workspace=None, template=None
    ):
        if report is None:
        ...
        template_file = (
            ...
            if template and template is not None
                ...
                self.dashboard_templates_directory, template_file + ".html"
            )
        )
        ...
        if not os.path.exists(destination):
            ...
            result.append("")
            result.append(
                'INFO: "--destination" {destination} path not exists and has been created.'.format(
        ...
        result.append(
            "INFO: {csv_report} file saved".format(csv_report=csv_files["csv_report"])
        )
        ...
        result.append(
            "INFO: {csv_errors} file saved".format(csv_errors=csv_files["csv_errors"])
        )
        ...
        result.append(
            "INFO: {csv_layers} file saved".format(csv_layers=csv_files["csv_layers"])
        )
        ...
        if report["type"].lower() in ["wms", "wfs"]:
            ...
            result.append(
                "INFO: {csv_ws} file saved".format(csv_ws=csv_files["csv_ws"])
            )
